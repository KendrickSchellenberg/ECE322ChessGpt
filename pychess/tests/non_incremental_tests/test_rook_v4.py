...
class TestRookGetValidPieceMoves(unittest.TestCase):
    ...
    def test_get_valid_piece_moves_all(self):
        rook = Rook('r', 1, 1, self.mock_white)
        with patch.object(rook, 'get_row_number', return_value=1), \
                patch.object(rook, 'get_col_number', return_value=1):
            moves = rook.get_valid_piece_moves(self.mocked_game_state)
        expected_moves = [
            (0, 1), (1, 0), (1, 2),  # Left, Above, Below
